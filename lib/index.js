...
/*
 * This maps the generic playername which is just
 * the corresponding number to a provided name.
 */
const mapNamesToObjectValues = (rounds, names) =>
  iterateOverRounds(rounds, ([key, round]) => [
    key,
    round.map((game) => {
      const keys = Object.keys(game);
      return {
        [keys[0]]: names[game[keys[0]] - 1],
        [keys[1]]: names[game[keys[1]] - 1],
      };
    }),
  ]);
...
const spielplan = (playerCount, config = {}) => {
  ...
  if (userConfig.mapPlayerNames) {
    schedule = mapNamesToObjectValues(schedule, userConfig.mapPlayerNames);
  }
  return schedule;
};
